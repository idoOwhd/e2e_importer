...
def logAndReturn(
    ...
    print("lyl-resultdict", resultdict)
...
def runOnnxToTorchMLIRGeneration(
    ...
    if SHARED_TORCH_MLIR_BUILD:
        ...
        print("lyl-runOnnxToTorchMLIRGeneration(SHARED_TORCH_MLIR_BUILD-1)-scriptcommand: ", scriptcommand)
        ...
        if launchCommand(args, scriptcommand, commandslog):
            ...
            print("lyl-runOnnxToTorchMLIRGeneration(SHARED_TORCH_MLIR_BUILD-2--Fail)-logAndReturn: ", commandslog,timelog,resultdict,1,uploadtestsList,cleanup,testName,uploadDict,dateAndTime,)
        ...
        print("lyl-runOnnxToTorchMLIRGeneration(SHARED_TORCH_MLIR_BUILD-2)-scriptcommand: ", scriptcommand)
        start = time.time()
        if launchCommand(args, scriptcommand, commandslog):
            print("Test", testName, "failed [" + curphase + "]")
            end = time.time()
        ...
        print("lyl-runOnnxToTorchMLIRGeneration(SHARED_IREE_BUILD)-scriptcommand: ", scriptcommand)
        start = time.time()
        if launchCommand(args, scriptcommand, commandslog):
            print("Test", testName, "failed [" + curphase + "]")
            end = time.time()
            resultdict[curphase] = ["failed", end - start]
            print("lyl-runOnnxToTorchMLIRGeneration(SHARED_IREE_BUILD--Fail)-logAndReturn: ", commandslog,timelog,resultdict,1,uploadtestsList,cleanup,testName,uploadDict,dateAndTime,)
            return logAndReturn(
                commandslog,
                timelog,
                resultdict,
...
def runTorchMLIRGeneration(
    ...
    print("lyl-runTorchMLIRGeneration(SHARED_IREE_BUILD)-scriptcommand: ", scriptcommand)
    start = time.time()
    curphase = phases[0]
    if launchCommand(args, scriptcommand, commandslog):
        print("Test", testName, "failed [" + curphase + "]")
        ...
        resultdict[curphase] = ["failed", end - start]
        print("lyl-runTorchMLIRGeneration(SHARED_IREE_BUILD--Fail)-logAndReturn: ", commandslog,timelog,resultdict,1,uploadtestsList,cleanup,testName,uploadDict,dateAndTime,)
...
def runCodeGeneration(
    ...
    if (
        ...
        return 1
    logfilename = curphase + ".log"
    print("lyl-logfilename: ", logfilename)
    commandname = ""
    if SHARED_IREE_BUILD:
        commandname = SHARED_IREE_BUILD + "/tools/"
    # Else pick from path
    commandname += (
        "iree-compile --iree-input-type=auto --iree-hal-target-backends=" # with all fusion in iree
        # "iree-compile --iree-input-type=auto --enable-fuse=false --iree-hal-target-backends=" # without fusion with named op
        # "iree-compile --iree-input-type=auto --enable-fuse=false --enable-ALL-fuse=false --iree-hal-target-backends=" # without fusion with elementwise op
        + args.backend
        + " --iree-cuda-target=sm_70 --output-format=vm-bytecode"
    )
    print("lyl-commandname: ",commandname)
    scriptcommand = (
        commandname
        + " "
        + torchmlirOutputfilename
    ...
    print("lyl-runCodeGeneration(SHARED_IREE_BUILD)-scriptcommand: ", scriptcommand)
    ...
    if launchCommand(args, scriptcommand, commandslog):
        ...
        resultdict[curphase] = ["failed", end - start]
        print("lyl-runCodeGeneration(SHARED_IREE_BUILD--Fail)-logAndReturn: ", commandslog,timelog,resultdict,1,uploadtestsList,cleanup,testName,uploadDict,dateAndTime,)
        return logAndReturn(
            commandslog,
            timelog,
            resultdict,
...
def runInference(
    ...
    print("lyl-runInference(SHARED_IREE_BUILD)-scriptcommand: ", scriptcommand)
    ...
    if launchCommand(args, scriptcommand, commandslog):
        ...
        resultdict[curphase] = ["failed", end - start]
        print("lyl-runInference(SHARED_IREE_BUILD--Fail)-logAndReturn: ", commandslog,timelog,resultdict,1,uploadtestsList,cleanup,testName,uploadDict,dateAndTime,)
        return logAndReturn(
            commandslog,
            timelog,
            resultdict,
    ...
    for i in range(0, len(goldoutputlist)):
        ...
        print("lyl-goldoutput",goldoutput)
        outputshape = goldoutput.size()
        print("lyl-outputshape",outputshape)
        torchdtype = goldoutput.dtype
        print("lyl-torchdtype",torchdtype)
        infoutputfilename = getinfoutfilename(i)
        print("lyl-infoutputfilename",infoutputfilename)
        if args.verbose:
            print(
...
def runTestUsingClassicalFlow(args_tuple):
    ...
    print("lyl-args_tuple", args_tuple)
    stubrunmodelpy = toolsDirAbsPath + "/stubs/pytorchmodel.py"
    print("lyl-stubrunmodelpy", stubrunmodelpy)
    ...
    if frameworkname == "pytorch":
        if mode == "turbine":
            ...
            print("lyl-onnxfilename: ",onnxfilename)
            torchmlirOutputfilename = (
                modelname + "." + args.todtype + ".pytorch" + torchmlirsuffix
            )
            print("lyl-torchmlirOutputfilename: ",torchmlirOutputfilename)         
            testargs += " --torchmlirimport " + args.torchmlirimport
            print("lyl-testargs: ",testargs)
    ...
    print("lyl-logfilename: ",logfilename)
    scriptcommand = (
        "/root/miniconda3/envs/shark/bin/python " + runmodelpy + " " + testargs + " 1> " + logfilename + " 2>&1"
    )
    print("lyl-scriptcommand: ",scriptcommand)
...
def runTest(aTuple):
    print("lyl-aTuple:",aTuple)
    curdir = os.getcwd()
    print("lyl-curdir:",curdir)
    ...
    print("lyl-testAbsPath:",testAbsPath)

    toolsDirAbsPath = script_dir + "/tools"
    print("lyl-toolsDirAbsPath:",toolsDirAbsPath)

    utilspy = toolsDirAbsPath + "/stubs/commonutils.py"
    print("lyl-utilspy:",utilspy)
    
    modelpy = testAbsPath + "/model.py"
    print("lyl-modelpy:",modelpy)
...
def runFrameworkTests(
    ...
    print("lyl-uploadDict: ",uploadDict)
    dateAndTime = str(datetime.datetime.now(datetime.timezone.utc))
    print("lyl-dateAndTime: ",dateAndTime)
    tupleOfListArg = []
    ...
    print("lyl-tupleOfListArg: ",tupleOfListArg)
    if args.verbose:
...
def checkBuild(run_dir, args):
    ...
    if args.ireebuild: #检查用户提供路径的正确性
        ...
        if not os.path.exists(IREE_BUILD):
            ...
            sys.exit(1)
        print("lyl-IREE_BUILD: ", IREE_BUILD)
    if not os.path.exists(run_dir):
        try:
            os.mkdir(run_dir)
            print("lyl-run_dir: ",run_dir)
        except OSError as errormsg:
            print(
...
def main():
    ...
    parser.add_argument(
        "-b",
        "--backend",
        choices=["llvm-cpu", "amd-aie", "rocm", "cuda"],
        default="llvm-cpu",
    ...
    parser.add_argument(
        ...
        default=["operators", "combinations", "models"],
    ...
    cache_dir = os.path.abspath(cache_dir) #规范化 cache_dir，确保它是一个绝对路径
    print("lyl-cache_dir: ", cache_dir)
    ...
    print("lyl-space_available: ", space_available)
    ...
    script_dir = os.path.dirname(os.path.realpath(__file__)) #当前正在执行的脚本文件的目录
    run_dir = os.path.abspath(args.rundirectory)
    frameworks = args.frameworks
    print("lyl-script_dir: ", script_dir)
    print("lyl-run_dir: ", run_dir)
    print("lyl-frameworks: ", frameworks)
    ...
    if args.tests or args.testsfile:
        ...
        print("lyl-frameworktotests_dict: ", frameworktotests_dict)
        for framework in frameworktotests_dict:
            ...
            if not args.norun:
                print("lyl-runFrameworkTests args: ", framework,",", testsList, ",", args, ",", script_dir, ",", run_dir, ",", TORCH_MLIR_BUILD, ",", IREE_BUILD)
                runFrameworkTests(
                    framework,
                    testsList,
                    args,
    ...
    # if args.uploadtestsfile: #从来不会考虑upload，哈哈哈
    #     print(
